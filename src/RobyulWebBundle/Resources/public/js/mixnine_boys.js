...
// Get data
d3.csv("/bundles/robyulweb/csv/mixnine.boys.v13.csv", parseLine, function (err, data) {
...
function getImageSource(d) {
    return "/bundles/robyulweb/images/mixnine/contestants/" + d.name.replace(/ /g, "") + ".jpg";
}
...
function displayRankChange(d) {
    if (d.rankChange == "-") {
        ...
        return "<img src='/bundles/robyulweb/images/idolschool/up-arrow.png' class='arrow'><span class='change up'>" + d.rankChange + '<span>';
    } else if (d.rankChange < 0) {
        return "<img src='/bundles/robyulweb/images/idolschool/down-arrow.png' class='arrow'><span class='change down'>" + Math.abs(d.rankChange) + '<span>';
    } else {
        return "<img src='/bundles/robyulweb/images/idolschool/neutral-arrow.png' class='arrow'><span class='change'>0</span>";
    }
}
...
function updateNotes(d) {
    ...
    for (var i = 0; i < episodes.length; i++) {
        ...
        if (d.ranking[i] == undefined) {
            ...
            $("#note" + i)
                .text(rank)
                .css("top", scaleY(rank) + OFFSET + TOP_OFFSET)
                .css("background", getBackground(d))
                .css("color", getTextColor(d));
        }
    }
...
function parseLine(row) {
    ...
    r.ranking = [];
    episodes.forEach(function(episode, i) {
        var rank = getRank(row["Ranking " + episode]);
        if (rank > 0) {
            var o = {};
            o.episode = episode;
            o.x = i;
